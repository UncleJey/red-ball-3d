...
#region PUBLIC_CLASSES
	...
	public GameObject getCollided(LayerMask groundLayer)
	{
		Collider2D cc;
		foreach(JellySprite.ReferencePoint referencePoint in ReferencePoints)
		{
			if(referencePoint.Collider2D)
			{
				CircleCollider2D circleCollider = referencePoint.Collider2D;
				Vector2 bodyPosition = referencePoint.transform.position;
				cc = Physics2D.OverlapCircle(bodyPosition + new Vector2(0, -circleCollider.radius * 0.1f), circleCollider.radius+0.5f, groundLayer);
				if(cc)
